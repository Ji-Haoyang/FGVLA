...
class CLIP(nn.Module):
    ...
    def __init__(self, embed_dim: int, vision_cfg: CLIPVisionCfg, text_cfg: CLIPTextCfg,
        ...
        self.adapter_img  = nn.ModuleList([Adapter(768, 8) for layer_id in range(12)])
        self.adapter_text = nn.ModuleList([Adapter(512, 8) for layer_id in range(12)])

        self.visual = _build_vision_tower(embed_dim, vision_cfg, quick_gelu, cast_dtype, adapter_img=self.adapter_img)
        text = _build_text_tower(embed_dim, text_cfg, quick_gelu, cast_dtype, adapter_text=self.adapter_text)
